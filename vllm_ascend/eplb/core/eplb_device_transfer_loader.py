...
class D2DExpertWeightLoader:
    ...
    def update_expert_map_and_weight(self, reqs):
        ...
        logger.info("[EPLB DEBUG] Waiting for %d D2D reqs for layer %d", len(reqs), self.layer_id)
        for req in reqs:
            req.wait()
        logger.info("[EPLB DEBUG] All %d D2D reqs completed for layer %d", len(reqs), self.layer_id)
