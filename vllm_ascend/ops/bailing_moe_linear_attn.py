...
def _import_linear_attn_fns():
    """Import linear attention functions, trying all known paths."""
    paths = []
    ...
    for path in paths:
        try:
            ...
            return (
                mod.clear_linear_attention_cache_for_new_sequences,
                mod.linear_attention_decode,
                mod.linear_attention_prefill_and_mix,
            )
        except (ImportError, ModuleNotFoundError):
            continue

    raise ImportError(
        "Cannot import linear attention functions. "
        "BailingMoELinearAttention requires a compatible vLLM version."
    )
